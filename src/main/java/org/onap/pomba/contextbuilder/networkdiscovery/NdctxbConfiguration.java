...
public class NdctxbConfiguration {
    ...
    public String getSdBasicAuth() {
        String auth = new String(
                this.serviceDecompositionUserId + ":" + Password.deobfuscate(this.serviceDecompositionPassword));
    ...
    @Value("${networkDiscoveryMicroService.password:admin}")
    ...
    public String getNetworkDiscoveryMicroServiceBasicAuth() {
        String auth = this.networkDiscoveryMicroServiceUserId + ":" + this.networkDiscoveryMicroServicePassword;
        return ("Basic " + Base64.getEncoder().encodeToString(auth.getBytes()));
    }
