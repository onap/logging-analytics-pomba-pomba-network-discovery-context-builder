...
public class NdctxbConfigurationTest {
 ...
 @Test
 public void testGetNetworkDiscoveryCtxBuilderBaseUrl() {
  String myUrl = "www.yahoo.com";
  when(configuration.getNetworkDiscoveryCtxBuilderBaseUrl()).thenReturn(myUrl);
  assertEquals(myUrl, configuration.getNetworkDiscoveryCtxBuilderBaseUrl());
 }

 @Test
 public void testGetJerseyClient() {
  JerseyConfiguration jerseyConfiguration = new JerseyConfiguration();
  Client myClient = jerseyConfiguration.jerseyClient();
  when(configuration.getJerseyClient()).thenReturn(myClient);
  assertEquals(myClient, configuration.getJerseyClient());
 }
}
